...
assert os.path.exists(limits_by_lang_cfg_file), (
...
def xcode_evaluate(generated_code: str, src_uid: str, lang: str):
    ...
    assert lang in LANGUAGE_MAPPING, (
        f"language must be inside the supported language list: {LANGUAGE_MAPPING.keys()}"
    )
...
def xcode_execute_internal_test(
    generated_code: str, tests: List[dict], src_uid: str, lang: str
):
    results, _, _ = api_comm.execute_code(
        language=LANGUAGE_MAPPING[lang],
        source_code=generated_code,
...
def contest_evaluate_public_tests(
    ...
    passed_feedback = (
        "\n".join(passed_feedback)
        if len(passed_feedback) > 0

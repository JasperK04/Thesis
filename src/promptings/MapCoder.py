    ...
    def parse_xml(self, response: str) -> dict:
        ...
        try:
            ...
            try:
                ...
                print(f"Error parsing XML (fallback): {e}")
                root = ET.fromstring("<root>\n" + response)
...
""",
        ...
        algorithm_prompt = (
            f"## Relevant Algorithm to solve the next problem:\n{response['algorithm']}"
        )
        sample_io_prompt = (
        ...
        if (
            isinstance(self.data, APPSDataset)
            or isinstance(self.data, CodeContestDataset)
            or isinstance(self.data, XCodeDataset)
        ):
